...
[vref(:,k),chi_estimados(:,k+1)] = adaptive_dymanic_UAV(vcp(:,k), vc(:,k), u_real(:,k), chi_estimados(:,k),chi, K1, K2, L, ts);
%vref(:,k) = dynamic_compensation_UAV(vcp(:,k), vc(:,k), u_real(:,k), chi, K1, K2, L, ts);
