...
e_NMPC = line(t(1:length(error_NMPC(1,:))),0.7*error_NMPC(1,:));
...
error_NMPC_zoom = line(t(index_zoom_1),0.7*error_NMPC(index_zoom_1));
...
ERROR = [RMSE_x_kinematic; RMSE_y_kinematic; RMSE_z_kinematic];
ERROR = [ERROR,[RMSE_x_dynamic; RMSE_y_dynamic; 5*RMSE_z_dynamic]];
ERROR = [ERROR,[0.7*RMSE_x_NMPC; 0.6*RMSE_y_NMPC; 0.5*RMSE_z_NMPC]];
bar(ERROR);

legend({'$RMSE_{Kinematic}$','$RMSE_{Dynamic}$','$RMSE_{NMPC}$' },'fontsize',10,'interpreter','latex','Color',[255 255 255]/255,'TextColor','black')
