...
for k = 1:length(t)
    ...
    error_kinematic(k) = norm(error_vector,2);
end
...
%% Colorbar
scatter(h(1, :),h(2, :),30,error_kinematic,'filled','MarkerFaceAlpha',0.7);
c = colorbar;
...
%% Colorbar
scatter(h(1, :),h(3, :),30,error_kinematic,'filled','MarkerFaceAlpha',0.7);
c = colorbar;
c.Label.Interpreter = 'latex';
c.Label.String = '$\textrm{Error}~||\tilde{\mathbf{\eta}}||$';
...
%% Legend nomeclature
hLegend_4 = legend([error_vx_plot,error_vy_plot, error_vz_plot],{'${\mu}_{l_{c}}$','${\mu}_{m_{c}}$','${\mu}_{n_{c}}$'},'fontsize',12,'interpreter','latex','Color',[255 255 255]/255,'Location','best','NumColumns',1,'TextColor','black');
 set(gca,'ticklabelinterpreter','latex',...
         'fontsize',1.3*fontsizeTicks)
%% Figure properties
ax_4 = gca;
...
export_fig Kinematics_real_results_b.pdf -q101

%% RMS kineamtics
RMSE_x_kinematic = sqrt(mean((error_vector(1,:)).^2));
RMSE_y_kinematic = sqrt(mean((error_vector(2,:)).^2));
RMSE_z_kinematic = sqrt(mean((error_vector(3,:)).^2));

save("Kinematics_real_rmse.mat","RMSE_x_kinematic", "RMSE_y_kinematic", "RMSE_z_kinematic", "error_kinematic");